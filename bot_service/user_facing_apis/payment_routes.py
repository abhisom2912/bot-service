...
# to accept payment via crypto currencies
payment_router = APIRouter()

erc20_abi = '[{"constant":true,"inputs":[],"name":"name","outputs":[{"name":"","type":"string"}],"payable":false,"type":"function"},{"constant":true,"inputs":[],"name":"decimals","outputs":[{"name":"","type":"uint8"}],"payable":false,"type":"function"},{"constant":true,"inputs":[{"name":"_owner","type":"address"}],"name":"balanceOf","outputs":[{"name":"balance","type":"uint256"}],"payable":false,"type":"function"},{"constant":true,"inputs":[],"name":"symbol","outputs":[{"name":"","type":"string"}],"payable":false,"type":"function"}]'
accepted_method = '0xa9059cbb'  # transfer function
accepted_address = 'YOUR_ERC20_ADDRESS'
...
def get_accepted_payment_info(chain_id):
    ...
    for accepted_token in chain_accepted_payment['accepted_tokens']:
        accepted_tokens[accepted_token['token_address'].lower()
                        ] = accepted_token['token_name']
...
def get_payment_details(chain_id, transaction_hash):
    chain_name, accepted_tokens, min_amount = get_accepted_payment_info(
        chain_id)
    file = open('./resources/rpc.json')
    chain_rpc = json.load(file)
    web3 = Web3(Web3.HTTPProvider(chain_rpc[chain_id]))

    # fetching user's payment information by querying the blockchain via web3.py
    ...
    # destination address should be your ERC20 address
    ...
    token_contract = web3.eth.contract(
        transaction_receipt['logs'][0]['address'], abi=erc20_abi)
...
# API to accept payment from the user against a protocol
...
def create_payment(protocol_id: str, chain_id: str, transaction_hash: str, request: Request):
    ...
    if request.app.database["payments"].find_one({"payment_details.transaction_hash": transaction_hash}):
        raise HTTPException(status_code=status.HTTP_409_CONFLICT,
                            detail=f"This txn hash already exists")

    payment_details = get_payment_details(chain_id, transaction_hash)
    payment_details['update_time'] = datetime.now()

    protocol_payments = request.app.database["payments"].find_one(
        {"protocol_id": protocol_id})
    if protocol_payments is None:
        payment = {'protocol_id': protocol_id, 'payment_details': [
            payment_details], "_id": uuid.uuid4()}
        ...
        request.app.database["payments"].update_one(
            {"protocol_id": protocol_id}, {'$set': protocol_payments})

    # adding credits to the user's account based on the amount paid
...
# API to fetch payment details using a transaction_hash
...
#     get_payment_details(
#         '137', '0x360de3b3a73678cf8bd952511f8ca66395a986b612ccf4682b1d535dc5ae79f3')
