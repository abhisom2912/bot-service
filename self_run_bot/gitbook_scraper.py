...
def get_gitbook_data(base_url, first_url):
    ...
    for page_url in all_page_urls:     # for every page in your documentation
        page = requests.get(page_url)
        ...
        # getting all the headings in your page
        ...
        while i < len(headings) - 1:
            ...
            title_stack.append(
                [header_level, headings[i].get_text(), content_text, page_url])
            i = i + 1
        ...
        for span in spans:
            content_text = content_text + span.get_text() + '\n'
        title_stack.append(
            [header_level, headings[i].get_text(), content_text, page_url])  # returning a list where each element contains the header level, the heading, its corresponding text, and the URL of the page
    return title_stack
