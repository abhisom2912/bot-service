...
# API to register a new protocol and save them in the database
...
def create_protocol(request: Request, protocol: Protocol = Body(...)):
    ...
    # checking if the user creating the protocol is already registered since only a registered user can create a new protocol
...
# API to fetch all protocols registered by a user
...
# API to fetch all the registered protocols present in the database
...
# API to fetch a specific protocol by its id
...
def find_protocol(user_id: str, id: str, request: Request):
    if (
        ...
        return protocol
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                        detail=f"Protocol with ID {id} not found")


# API to update a specific protocol using its id
...
# API to save the response of a user's Discord moderator in the database
# this can be used to answer questions that the bot itself can't
...
# API to delete a registered protocol from the database
